...
@click.command()
@click.option('--pipeline1', '-o', required=True, type=str, help='Path to output directory from CPAC run '
              'to correlate against pipeline2')
@click.option('--pipeline2', '-o', required=True, type=str, help='Path to output directory from CPAC run '
...
def correlate(pipeline1, pipeline2, workspace):
    ...
    parse_yaml.cpac_yaml(pipeline1, pipeline2, f'{workspace}/correlations', 'run', 1)
